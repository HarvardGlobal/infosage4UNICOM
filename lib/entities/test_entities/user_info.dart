...
class UserInfo extends ChangeNotifier {
  ...
  set medicationsList(List<Medication>? value) {
    _medicationsList = value;
    notifyListeners();
  }

  set languageSelected(bool value) {
  ...
  UserInfo.fromMap(Map<String, dynamic> userInfoMap) {
    if (userInfoMap['languageSelected'] != null) {
      _languageSelected = userInfoMap['languageSelected'];
    }
    if (userInfoMap['darkMode'] != null) {
      _darkMode = userInfoMap['darkMode'];
    }
    if (userInfoMap['fontSize'] != null) {
      _fontSize = userInfoMap['fontSize'];
    }
    if (userInfoMap['name'] != null) {
    ...
    if (userInfoMap['medicationsList'] != null) {
      for (Map<String, dynamic> medicationMap
          in userInfoMap['medicationsList']) {
  ...
  void removeCorrelated(String medicationId, List<Medication> medications) {
    _correlated?[medicationId]
        ?.removeWhere((item) => medications.contains(item));
    notifyListeners();
  }

  void addCorrelated(String medicationId, List<Medication> correlated) {
    if (medicationsList!
        .where((element) => element.id == medicationId)
        .isEmpty) {
      throw AssertionError('$medicationId is not in medication list');
    }
    List<Medication> medicationCorrelated =
        _correlated![medicationId] ?? <Medication>[];
  ...
  bool hasCorrelated(String medicationId, Medication medication) {
    return _correlated?[medicationId] != null &&
        _correlated![medicationId]!.contains(medication);
  }
